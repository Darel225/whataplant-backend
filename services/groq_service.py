"""
services/groq_service.py — Analyse IA avec Groq Vision
=======================================================
Utilise les modèles vision de Groq pour analyser les plantes.
"""

import requests
import json
import base64
from config import GROQ_API_KEY, GROQ_URL, GROQ_VISION_MODELS


def analyser_plante(nom_scientifique: str, image_bytes: bytes) -> dict:
    """
    Envoie le nom scientifique + l'image à Groq Vision.
...
Pour les noms locaux, donne UNIQUEMENT le nom français le plus courant en Côte d'Ivoire.
...
{{
  ...
  "mode_utilisation": "Explication détaillée de la préparation",
...
Réponds UNIQUEMENT avec le JSON, sans texte avant ou après."""
    ...
    for modele in GROQ_VISION_MODELS:
        payload = {
            "model": modele,
            "messages": [
                {
        ...
        print(f"  Appel API Groq avec modèle: {modele}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  Groq réponse reçue ({len(contenu)} caractères)")
                print(f"  Contenu brut: {contenu[:200]}...")  # ← AJOUTE CETTE LIGNE
    ...
    print("  ⚠️  Aucun modèle Groq fonctionnel, utilisation fallback")
    return _reponse_defaut(nom_scientifique)
